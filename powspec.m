...
if nargin < 3
  wintime = 0.03;                      % Original code - wintime = 0.025;
end
if nargin < 4
  steptime = 0.00006237;               % (479/480)/16000 = 0.00006237       % Original code - steptime = 0.01;
end
if nargin < 5
  dither = 1;
end                        
...
NFFT = 1024;                            % Original code - NFFT = 2^(ceil(log(winpts)/log(2)));    
WINDOW = hamming(winpts);
%WINDOW = [0,hanning(winpts)'];
%WINDOW = hanning(winpts)';             % Original code - WINDOW = hanning(winpts)';     % hanning gives much less noisy sidelobes
